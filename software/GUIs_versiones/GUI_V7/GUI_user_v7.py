...
from dispositivo_serial import criogenia_serial
import time

# Configurar la comunicación serial con los Arduinos
myinstrument = criogenia_serial("COM19")

myinstrument.conectar()


# Función para obtener los datos de temperatura del Arduinos
def obtener_temperaturas():
    temp = myinstrument.read_temp()
    if temp is not None:
        tu = temp.get('tu')
        td = temp.get('td')
...
def control_motor(temp2):
     if int(temp2) > 2048:
         time.sleep(5)
         myinstrument.motor(-0.1)  # Enviar la orden al motor de paso
...
def enviar_numero_motor():
    ...
    myinstrument.motor(numero)  # Enviar el número al motor
    time.sleep(0.1)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.timer.start(200)  # Actualizar cada 500 ms
